...
1. npm run dev - Собирает сервер, запускает его, а на сервере собирается клиент с HMR (в папку dist это не попадает)  
    1.1 npm run dev:docker Поднимает базу данных и ее админку в докере
    1.2. npm run dev:webpack - Собирает сервер  
    1.3. npm run dev:server - Запускает сервер, собирает клиент с HMR (в папку dist это не попадает)  
...
6. npm run dockerdown - Останавливает и удаляет все докер контейнеры, которые используются в нашем приложении
